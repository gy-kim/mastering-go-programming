...
func main() {
	...
	vpElement.SetFloat(2.2)
	fmt.Println(x1)

	// For simplicity, the getter provides the largest type of a value. So instead of float32 it returns float32 it returns float64
	x2 := v.Float()
...
func inspectTypeFloat(i interface{}) {
	v := reflect.ValueOf(i)
	fmt.Println("type:", v.Type())
	fmt.Println("Kind is float64?", v.Kind() == reflect.Float64)
	fmt.Println("Value:", v.Float())
