...
func main() {
	...
	*/
	...
	f3, err := os.OpenFile("test3.txt", os.O_APPEND|os.O_WRONLY, 0666)
	// os.O_RDONLY // ReadOnly
	// os.O_WRONLY // WriteOnly
	// os.O_RDWR   // Read and write
	// os.O_APPEND // Append to end of file
	// os.O_CREATE // Create is none exist
	// os.O_TRUNC // Trucate ifle when opening
	...
	// buffered write, efficient store in memory, saves dis I/O
	writebuffer := bufio.NewWriter(f3)
	for i := 0; i <= 5; i++ {
		writebuffer.WriteString(fmt.Sprintln("Add line", i))
	}
	writebuffer.Flush()
