...
func main() {
	db, err := sql.Open("mysql", "Put Connection string")
	if err != nil {
		log.Fatal("Could now connect, error", err.Error())
	}
	defer db.Close()
...
func GetCrewByPositions(db *sql.DB, positions []string) Crew {
	Qs := fmt.Sprintf("SELECT id, Name, SecurityClearance, Position from Personnel where Postion in (%s);", strings.Join(positions, ","))

	rows, err := db.Query(Qs)
	if err != nil {
		log.Fatal("Could now get data from the Personnel table ", err)
	}
	defer rows.Close()
...
func CreateCrewMembersByTransaction(db *sql.DB, cr Crew) {
	tx, err := db.Begin()
	if err != nil {
		log.Fatal("Could now begin tx", err)
	}
